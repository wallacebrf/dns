...
block_geo=1
Working_Dir="/var/www"
source_warning_counter=0
source_warning_limit=20
...
date  |& tee -a "$Working_Dir/log/$date.txt" 

#check that the script is running as root or some of the commands required will not work
if [[ $( whoami ) != "root" ]]; then
	echo -e "ERROR - Script requires ROOT permissions, exiting script"  |& tee -a "$Working_Dir/log/$date.txt" 
	exit 1
fi
...
if [[ $block_ASN -eq 1 ]]; then
	...
	let num_ASN=num_ASN+1
	counter=1
	
	manual_block_list="https://raw.githubusercontent.com/wallacebrf/dns/refs/heads/main/manual_block_list.txt"
	
	
	echo "ASN $counter/$num_ASN - Processing \"$manual_block_list\""  |& tee -a "$Working_Dir/log/$date.txt"
	let counter=counter+1
	curl -s "$manual_block_list" > "$Working_Dir/tmp/ASN$counter.txt"
	num_lines=$(wc -l < "$Working_Dir/tmp/ASN$counter.txt")
	if [ "$num_lines" -gt 0 ]; then
		echo -e "\n$num_lines Subnets Downloaded from $manual_block_list\n"  |& tee -a "$Working_Dir/log/$date.txt"
	else
		echo -e "\nWARNING - $manual_block_list Returned Zero Subnets\n"  |& tee -a "$Working_Dir/log/$date.txt"
		let source_warning_counter=source_warning_counter+1
	fi
		
	while read -r line; do
		echo "ASN $counter/$num_ASN - Processing \"${line//[$'\t\r\n ']}\""  |& tee -a "$Working_Dir/log/$date.txt"
		let counter=counter+1
		curl --ipv4 -s "https://asn.ipinfo.app/api/text/list/${line//[$'\t\r\n ']}" > "$Working_Dir/tmp/ASN$counter.txt"
		num_lines=$(wc -l < "$Working_Dir/tmp/ASN$counter.txt")
		if [ "$num_lines" -gt 0 ]; then
...
#supply of IPs for different countries: https://herrbischoff.com/code/me/country-ip-blocks/src/branch/master/ipv4
...
if [[ $block_geo -eq 1 ]]; then
	...
	while read -r line; do
		...
		#wget --inet4-only -q -O "$Working_Dir/geo$counter.txt" "${line//[$'\t\r\n ']}"
		#mv "$Working_Dir/geo$counter.txt" "$Working_Dir/tmp/geo$counter.txt"
		curl -s "${line//[$'\t\r\n ']}" > "$Working_Dir/tmp/geo$counter.txt"
...
##########################################################################
# Ensure that we have sucessfully downloaded data
##########################################################################
if [[ $source_warning_counter -ge $source_warning_limit ]]; then
	echo -e "\n\n\n\n\n!!!!!ATTENTION!!!!! - $source_warning_counter data source entries have returned zero result, more than the allowable limit of $source_warning_limit, cancelling script\n\n"  |& tee -a "$Working_Dir/log/$date.txt"
	exit 1
fi
...
/var/www/aggregate6 "$Working_Dir/tmp/master_sorted.txt" > "$Working_Dir/tmp/master.txt"
num_lines=$(wc -l < "$Working_Dir/tmp/master.txt")
...
sleep 10
echo "110 seconds remaining"
sleep 10
echo "100 seconds remaining"
sleep 10
echo "90 seconds remaining"
sleep 10
echo "80 seconds remaining"
sleep 10
echo "70 seconds remaining"
sleep 10
	...
	if grep -wq "$block" "$Working_Dir/tmp/current_ufw.txt"; then 
		...
		if [[ "$block" == *":"* ]]; then
			if [[ "$ipv6" -eq 0 ]]; then
				...
				echo "Inserting NEW IPv6 address \"$block\"" |& tee -a "$Working_Dir/log/$date.txt"
			...
			echo "Inserting NEW IPv4 address \"$block\"" |& tee -a "$Working_Dir/log/$date.txt"
			if [[ "$test_mode" -eq 0 ]]; then
				ufw insert 1 deny from "$block"
			else
				echo "Script in Test Mode"
...
do
	...
	if grep -wq "$string" "$Working_Dir/tmp/master.txt"; then 
		...
		if [[ "$block2" == *"ALLOW IN"* ]]; then #if the current UFW configuration line is for the ALLOWED IN lines, do not touch those. 
			echo "Skipping removal of line \"$block2\" as this is not part of the ASN blocking" |& tee -a "$Working_Dir/log/$date.txt"
		elif [[ "$block2" == *"DENY IN"* ]]; then
			echo "Removing un-needed UFW address \"$string\"" |& tee -a "$Working_Dir/log/$date.txt"
			if [[ "$test_mode" -eq 0 ]]; then
			...
			echo "skipping unknown data \"$block2\"" |& tee -a "$Working_Dir/log/$date.txt"
		fi
	fi
	let counter=counter+1
...
cd "$Working_Dir/log"
current_dir=$(pwd)
if [ "$current_dir" = "$Working_Dir/log" ]; then
	echo "Cleaning up log  directory \"$Working_Dir/log\"" |& tee -a "$Working_Dir/log/$date.txt"
	ls -1t | tail -n +31 | xargs rm -f
else
	echo "Could not change to log directory, canceling cleaning of log directory \"$Working_Dir/log\"" |& tee -a "$Working_Dir/log/$date.txt"
fi
