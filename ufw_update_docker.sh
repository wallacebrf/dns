...
# Edit /etc/ufw/after.rules with the following content from https://github.com/chaifeng/ufw-docker?tab=readme-ov-file#how-to-do to enable ufw blocking in docker containers 
# Copy without the first #:
...
echo -e "***************************************\n\n" |& tee -a "$Working_Dir/log/$date.txt"
sudo ufw status numbered | tee "$Working_Dir/tmp/current_ufw.txt"
...
do
	...
	in_rule=$(  grep -w "DENY IN.*[[:space:]]$block\$"  "$Working_Dir/tmp/current_ufw.txt" || true)
	fwd_rule=$( grep -w "DENY FWD.*[[:space:]]$block\$" "$Working_Dir/tmp/current_ufw.txt" || true)
	if [[ -n "$in_rule" && -n "$fwd_rule" ]]; then
		...
		if [[ "$block" == *":"* ]]; then
            ...
            [[ -z "$fwd_rule" ]] && {
                echo -n "adding DENY FWD …  "
                [[ "$test_mode" -eq 0 ]] && sudo ufw route insert 1 deny from "$block"
            }
            [[ -z "$in_rule" ]] && {
                echo -n "adding DENY IN …  "
                [[ "$test_mode" -eq 0 ]] && sudo ufw insert 1 deny from "$block"	
			}
			echo
			fi
		################
...
do
	...
	if grep -wq "$string" "$Working_Dir/tmp/master.txt"; then 
		...
		if [[ "$block2" == *"ALLOW IN"* || "$block2" == *"ALLOW FWD"* ]]; then #if the current UFW configuration line is for the ALLOWED IN lines, do not touch those. 
			echo "Skipping removal of line \"$block2\" as this is not part of the ASN blocking" 
		elif [[ "$block2" == *"DENY IN"* || "$block2" == *"DENY FWD"* ]]; then
			echo "Removing un-needed UFW address \"$string\""
			if [[ "$test_mode" -eq 0 ]]; then
				sudo ufw route delete deny from "$string" 2>/dev/null || true			
				sudo ufw delete deny from "$string" 2>/dev/null || true
			else
